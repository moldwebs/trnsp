$(document).ready(function(){
    ...
    $(document).ajaxSend(function(event, xhr, options) {
        if($('.ajax_loader').length == 0){
            //$('body').append('<div class="modal-backdrop fade in"><img style="top: 10%; left: 45%; position: absolute; opacity: 1;" src="/assets/img/loading.gif"></div>');
            $('<div class="ajax_loader"><img style="top: 10%; left: 45%; position: absolute; opacity: 1;" src="/assets/theme_orig/assets/img/loading.gif"></div>').appendTo(document.body);
        }
    }).ajaxStop(function() {
        $(".ajax_loader").remove();
    }).ajaxError(function(event, xhr, options, thrownError){
        console.log('ajaxError: ' + thrownError);
		if(thrownError == 'Forbidden') window.location = '/';
    });
        

    $('[mx_ajx_win]').on('click', function(){
        $.get($(this).attr('mx_ajx_win'), function(data){
            bootbox.dialog({
              message: data,
              title: "&nbsp;",
              width: 900
            });
            $('.bootbox .panel-border-color').removeClass('panel-border-color');
            after_ajax();
        });
    });
    $('.mx_ajx_win,.ajx_win1').on('click', function(){
    ...
    $('[mx_href]').on('click', function(){
        window.location = $(this).attr('mx_href');
    });
    $('[mx_href_confirm]').on('click', function(){
        if(confirm('CONFIRM')){
            window.location = $(this).attr('mx_href_confirm');
        }
        return false;
    });
    $('.mx_href_confirm').on('click', function(){
    ...
    $('form.ajx_validate').on('submit', function(){
        if(!$(this).hasClass('ajx_validate')) return true;
        var obj = this;
        jQuery.ajax({
            async: true,
            type: 'post',
            success: function(data) {
                if(data.substring(0, 10) == '[function]'){
                    eval(data.substring(10));
                } else if(data == 'OK'){
...
function after_ajax(){
    ...
    /*
    $('.datepicker:not([readonly="readonly"])').attr('readonly', 'readonly').datepicker({
        dateFormat: 'yy-mm-dd'
	});
    $('.datepicker_future:not([readonly="readonly"])').attr('readonly', 'readonly').datepicker({
        dateFormat: 'yy-mm-dd',
        minDate: '0'
	});

    $('.timepicker:not([readonly="readonly"])').attr('readonly', 'readonly').timepicker({showPeriodLabels: false});
    */
    
    if($('[ajx_select]').length > 0){
        $('[ajx_select]').each(function(){
            var obj = this;
            $($(this).attr('ajx_select')).bind("change", function() {
                jQuery.ajax({
                    async: true,
                    type: 'post',
                    success: function(data) {
                        jQuery(obj).html(data).trigger('change');
                        if($(obj).attr('ajx_selected') != '' && $(obj).attr('ajx_selected') != undefined){
                            $(obj).val($(obj).attr('ajx_selected'));
                        } else {
                            $(obj).val($("#" + $(obj).attr('id') + " option:first").val());
                        }
                    },
                    data: jQuery(obj).parents('form:first').serialize() + '&ajx_select=' + jQuery(obj).attr('id'),
                    url: jQuery(obj).parents('form:first').attr('action')
                });
            });
            if($($(this).attr('ajx_select')).val() != '' && $($(this).attr('ajx_select')).val() != undefined) $($(this).attr('ajx_select')).trigger('change')
        });
    }

    $(".datetimepicker_win").datetimepicker({
    	autoclose: true,
